...
export default async function ScreenerPage() {
  let initial: Awaited<ReturnType<typeof getScreener>> | null = null;
  let apiError: string | null = null;
  let dataError: string | null = null;
  try {
    initial = await getScreener();
    if (initial && "error" in initial) {
      dataError = (initial as { message?: string }).message ?? "初筛数据缺失";
      initial = null;
    }
  } catch {
    initial = null;
  ...
  return (
    <div className="page-wrapper page-wrapper-wide">
      ...
      <ScreenerDashboard initial={initial} />
    </div>
  );
}
