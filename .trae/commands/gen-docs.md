...
文档是给**用户（股票投资者）**看的，不是给开发者看的。所有文档（含既有与新增）必须：

- 用投资者能直接理解的语言：系统是做什么的、页面 / 数字 / 图表怎么读、研报怎么保证可信
- **禁止罗列技术细节**：架构、端口、API 端点、脚本名、内部文件路径、frontmatter 字段名、本地启动命令、缓存 TTL 等一律不写
- 内部实现需提及时用面向用户的语言转述（如「按固定权重实时计算」而非「权重在 `composite.ts` 中」）

### 1. 明确范围

- 若 `$ARGUMENTS` 指定具体主题（如「投资理念」），只更新对应文档
- 若 `$ARGUMENTS` 为空或「全部」，全量审阅并更新四篇
- 若 `$ARGUMENTS` 为受众 / 写作风格类要求（如「给投资者看」「不要技术细节」），视为全量审阅并更新四篇，并按该要求统一写作风格
