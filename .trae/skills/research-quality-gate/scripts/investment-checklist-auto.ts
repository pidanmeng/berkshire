...
function autoScan(content: string, companyContent?: string): {
  ...
  // 4 AUTO: 三层结构（行业/细分/公司）——真实双链或正文章节均可；排除模板占位（一级行业/细分行业/竞争对手等占位符）
  // 行业层：frontmatter industry 双链 或 正文「行业全景/概览/分析/级」；细分层：frontmatter sub_industry 双链 或 正文「细分行业/赛道/细分」；公司层：type:company 或 正文「公司研究/公司概览/公司简介/代表公司」
  const hasThreeTiers =
    !has(/\[\[(?:一级行业|细分行业|竞争对手[AB]|上游[AB]|下游[AB]|公司名称)\]\]/i) && (
      has(/industry\s*:\s*\[\[[^\]]+(?:行业|产业)[^\]]*\]\]|行业(?:全景|概览|分析)|00-行业概览|行业级/i) &&
      has(/sub_industry\s*:\s*\[\[[^\]]+\]\]|细分行业|01-细分行业|赛道|细分/i) &&
      has(/type\s*:\s*"company"|公司研究|02-公司研究|代表公司|公司概览|公司简介/i)
    );
  _r(4, hasThreeTiers, true, hasThreeTiers ? "检测到三层结构（行业+细分+公司）" : "未完整发现三层结构，请检查知识库三层覆盖");

  // 8 AUTO: 是否下载了财报PDF（强证据：pdf-texts 路径引用 / 三年年报覆盖表述 / annual_reports_covered frontmatter；弱化通用「年报/PDF」宽松命中）
  const pdfDownloaded =
    has(/pdf-texts[^\n]*(?:\.md|\/)|(?:年报|半年报|季报)[^\n]*pdf-texts|pdf-texts[^\n]*(?:年报|半年报|季报)/i) ||
    has(/annual_reports_covered\s*[:：]\s*20\d{2}/i) ||
    has(/三(?:份|年)(?:度)?(?:年报|年度报告)[^\n]*(?:已|均)?(?:下载|精读|覆盖|齐全)/i) ||
    has(/(?:年报|半年报|季报)(?:PDF|pdf)[^\n]*(?:提取|解析|下载|已抓取)|(?:提取|解析|下载|已抓取)[^\n]*(?:PDF|pdf)[^\n]*(?:年报|半年报|季报)/i);
  _r(8, pdfDownloaded, true, pdfDownloaded ? "检测到财报PDF下载/解析痕迹（pdf-texts 或三年年报覆盖）" : "未检测到财报PDF下载记录");
  ...
  // 28 AUTO: 有息负债 / 经营现金流 > 5（阈值比值判定：解析金额计算比值，结合货币资金覆盖判断；无法解析时仅强关键词触发，否则标注需人工核验）
  const parseYi = (re: RegExp): number | null => {
    const m = re.exec(combined);
    if (!m) return null;
    const v = parseFloat(m[1]);
    if (isNaN(v)) return null;
    return m[0].includes("万") ? v * 1e-4 : v; // 万元 → 亿元
  };
  const interestDebtYi = parseYi(/(?:有息负债|带息负债|计息负债)\s*(?:合计)?[^\d]{0,10}([\d.]+)\s*(?:亿|万元?)/i);
  const ocfYi = parseYi(/(?:经营活动(?:产生)?的?现金流量净额|经营现金流(?:量)?(?:净额)?|OCF|经营性现金流)\s*(?:为|是|约|达)?[^\d]{0,10}([\d.]+)\s*(?:亿|万元?)/i);
  const cashYi = parseYi(/(?:货币资金|期末现金及现金等价物余额|现金及现金等价物|现金余额)[^\d]{0,10}([\d.]+)\s*(?:亿|万元?)/i);
  let r28ok = true, r28checked = false, r28note = "";
  if (interestDebtYi !== null && ocfYi !== null && ocfYi > 0) {
    const ratio = interestDebtYi / ocfYi;
    const cashCover = cashYi !== null && cashYi >= interestDebtYi * 0.8;
    r28ok = ratio <= 5 || cashCover;
    r28checked = true;
    r28note = ratio > 5
      ? `有息负债${interestDebtYi.toFixed(1)}亿 / OCF ${ocfYi.toFixed(1)}亿 = ${ratio.toFixed(2)}${cashCover ? `，货币资金${cashYi.toFixed(1)}亿可覆盖` : ""} ${r28ok ? "✅ 现金覆盖充足" : "🚨 有息负债/OCF>5 流动性红线"}`
      : `有息负债${interestDebtYi.toFixed(1)}亿 / OCF ${ocfYi.toFixed(1)}亿 = ${ratio.toFixed(2)}（<5）✅`;
  } else if (interestDebtYi !== null && ocfYi !== null && ocfYi <= 0) {
    r28ok = false; r28checked = true;
    r28note = `🚨 经营现金流为负（${ocfYi.toFixed(1)}亿），有息负债${interestDebtYi.toFixed(1)}亿无法覆盖`;
  } else if (has(/(债务压顶|违约风险|流动性.*(?:紧|恶化|枯竭)|到期.*无法(?:偿还|兑付))/i)) {
    r28ok = false; r28checked = true;
    r28note = "🚩 检测到债务风险强关键词（债务压顶/违约风险/流动性紧张）";
  } else {
    r28ok = true; r28checked = false;
    r28note = "未解析到有息负债/经营现金流金额，需人工核验有息负债÷OCF 比值（>5 为流动性红线）";
  }
  _r(28, r28ok, r28checked, r28note);
  ...
  // 30 AUTO: 大额减值 / 净利 > 30%（否定语境排除；数值化判定优先；纯关键词须带金额/占比）
  const impairNegation = has(/无(?:大额|重大)?(?:资产|商誉)?减值|未计提(?:.*)?减值|减值(?:损失|准备)?(?:已)?(?:转回|冲回)|(?:转回|冲回).*减值|减值.*(?:转回|冲回|通过|未触发)|不存在.*减值|未发生.*减值/i);
  const impairAmountPct = (() => {
    const m = combined.match(/(?:资产减值损失|信用减值损失|商誉减值|减值损失|大额减值|减值)[^\d]{0,16}([\d.]+)\s*(?:亿|万元?)[^\d]{0,60}?([\d.]+)\s*%/i);
    if (!m) return null;
    return parseFloat(m[2]);
  })();
  const impairRed = !impairNegation && (
    (impairAmountPct !== null && !isNaN(impairAmountPct) && impairAmountPct > 30) ||
    (!impairNegation && has(/大额减值[^\d]{0,20}[\d.]+[^\d]{0,6}(?:亿|万元?)/i))
  );
  _r(30, !impairRed, true,
    impairRed
      ? `🚩 大额减值${impairAmountPct !== null ? `占净利 ${impairAmountPct.toFixed(1)}%` : "警示"}（>30% 净利）`
      : (impairNegation ? "✅ 减值为转回/未计提/无减值语境，不触发警示" : "未检测到大额减值"));
