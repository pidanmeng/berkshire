...
> **子 Agent 触发契约（重要）**：当前执行环境（TRAE/Codex 类 Task 工具）的 `subagent_type` 白名单仅接受 `search` 与 `general_purpose_task`，上表所列专用 subagent\_type（`document-reader`/`info-alchemist`/`knowledge-architect`/`report-writer` 等）在多数运行环境无法直启（报错「subagent\_type is not a valid value」）。**统一约定**：
>
...
> 4. 若运行环境开放自定义 subagent\_type，可恢复直调（本契约同步更新）。
...
* **deep-dive 模式 raw\_source 约定**：deep-dive 流程无 Phase 1（不产出 raw 文件），deep-read/processed 的 frontmatter `raw_source` **禁止指向不存在的** **`[[YYYY-MM-DD-公司名-raw]]`**，应置空或默认链接 `pdf-texts/<公司名>/` 目录下实际存在的年报/研报 Markdown（`[[pdf-texts/公司名/2025年年度报告]]` 或直接标注目录路径）；标准 `/research` 流程保持指向 raw 文件
...
| 指令                           | 用途                                                   | 可选参数                                                                                           | <br /> | <br />    | <br />            | <br />                                                         |
| ---------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------- | :----- | :-------- | :---------------- | :------------------------------------------------------------- |
| `/research [主题]`             | 完整投研流程                                               | `--mode standard/fast/deep/incremental` `--decision-critical` `--company-only` `--sector-only` | <br /> | <br />    | <br />            | <br />                                                         |
| `/search [查询]`               | 仅 Phase 1                                            | —                                                                                              | <br /> | <br />    | <br />            | <br />                                                         |
| `/validate [文件]`             | 仅 Phase 3                                            | —                                                                                              | <br /> | <br />    | <br />            | <br />                                                         |
| `/report [主题]`               | 基于已有知识生成报告                                           | —                                                                                              | <br /> | <br />    | <br />            | <br />                                                         |
| `/revise [报告] [意见]`          | 修订报告                                                 | —                                                                                              | <br /> | <br />    | <br />            | <br />                                                         |
| `/sync-links`                | 修复断链与缺失 properties                                   | —                                                                                              | <br /> | <br />    | <br />            | <br />                                                         |
| `/update-moc`                | 更新 MOC 索引                                            | —                                                                                              | <br /> | <br />    | <br />            | <br />                                                         |
| `/quality-report [月份]`       | 生成质量趋势报告                                             | `--month YYYY-MM`                                                                              | <br /> | <br />    | <br />            | <br />                                                         |
| `/screen`                    | A 股全市场初筛（脚本为主，0 token；结果见 valuation-tracker「全市场初筛」页） | `--report YYYY-N` `--min-mcap` `--only a\|b\|c` `--codes` `--smoke`                            | <br /> | <br />    | <br />            | <br />                                                         |
...
| 脚本                                             | 用途                                                                                                                          | 用法                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.trae/scripts/stock-data/stock.ts`            | 证券代码 → 公告 / 券商研报 / 定期报告（年报、半年报、季报）PDF                                                                                       | `bun run .trae/scripts/stock-data/stock.ts --name <公司名>` 名称→代码（可追加下方操作参数，自动匹配首个结果）；`--code <代码>` 直接按代码查询。操作参数：`--announcements` 公告（默认近 90 天）、`--reports` 券商研报、`--financial` 定期报告 PDF。可选：`--category yjyg\|yjbb\|ndbg\|bndbg\|yjdbg\|sjdbg\|zqbg` 过滤公告类别（yjyg 业绩预告、yjbb 业绩快报、ndbg 年报、bndbg 半年报、yjdbg 一季报、sjdbg 三季报、zqbg 债券公告；`--financial` 自动覆盖 年报/半年报/一季报/三季报 四类）、`--days N` 调整时间范围                                                                                                                                         |
...
| `.trae/scripts/backtest/backtest.ts`           | 基本面精选指数回测（**严格 point-in-time**：历史时点全市场筛查 Top10 等权 → 每年 4/30、8/31、10/31 三季调仓 → 后复权收益 + 沪深300 基准），输出接入 valuation-tracker「回测」页 | `bun run .trae/scripts/backtest/backtest.ts` 完整回测 2020-01 起；`--start/--end/--year` 限定区间；`--smoke N --max-periods M` 冒烟（每期仅前 N 只主池）；`--min-mcap`、`--concurrency`。产物：`Research/00-Workspace/08-Backtest/backtest-result.json`（前端事实源）+ `<asof>-screener.json/.csv`（每期筛查）+ `cache/historical-<report>_<prev>.jsonl`（指标/TTM 缓存断点续跑）                                                                                                                                                                                              |
| `.trae/scripts/backtest/market-db.ts`          | 历史行情本地库（同花顺 dump 全市场 10 年日K + 复权因子 → DuckDB），提供收盘价/后复权序列/股本回推因子，回测数据层                                                       | `--download [daily-k\|adjustment-factors]` 下载 Parquet；`--build` 建库；`--status` 库状态；`--check <code> [date]` 抽查。库：`Research/00-Workspace/08-Backtest/market/market.duckdb`（依赖 `@duckdb/node-api`，已装于 `.trae/`）                                                                                                                                                                                                                                                                                                                 |
| `.trae/scripts/backtest/point-in-time.ts`      | 历史时点估值（PE/TTM EPS/市值，只用调仓日已披露信息），供筛查层 import                                                                                | `--check --code <代码> --asof <YYYY-MM-DD> --report <YYYY-N>` 输出该时点 PE/PB/市值（验证用）                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `investment-checklist`（50+ 检查项，分 CRITICAL/IMPORTANT/NORMAL） | 50 项分三级检查，CRITICAL > 1 项不发布，No-Go 条款一票否决  | **完全落地**：• 模板：`Research/99-Templates/50-item-investment-checklist.md`• Auto 脚本：`.trae/skills/research-quality-gate/scripts/investment-checklist-auto.ts`• Phase 5 附录必附 AUTO 扫描块 |
...
* [ ] **公司笔记/报告含「估值模型与参数明细」**：正文含模型选择（品种→主估值模型）+ 三情景参数取值依据（预测期净利/估值倍数，写明锚定方法如 PEG/同业/历史分位三重锚定）+ 参数敏感性（政策/原材料/竞争/汇率/技术代际）；frontmatter `valuation_model` 已回填且与 `target_market_cap_yi` / `forward_pe` / `peg` 一致

* [ ] **deep-dive-update 产物含「本期财报 vs 市场预期」评估**：业绩 vs 预期（预告区间中值/一致预期/去年同期）+ 股价反应（披露前后相对大盘/板块超额）+ 综合判定（超预期/符合/低于预期）
