# 黑金（Black & Gold）— Style Reference

> A black trading workspace with precise white type and one functional gold signal.

**Theme:** dark

This is a dark interpretation of Vercel's Geist interface language, re-tinted from its original blue to a black-and-gold palette. The canvas is absolute black, the hierarchy is carried by closely spaced charcoal surfaces and thin borders, and white text is reserved for the highest-priority information. Gold is an interaction signal rather than decoration: use it for focus, links, and the occasional featured action. The visual result should feel like a clear investment research console, not a glowing sci-fi dashboard.
...
| Gold | `#d4af37` | `--color-accent` | Links, focus rings, and featured actions |
| Gold Hover | `#e6c25c` | `--color-accent-hover` | Hover state for gold actions |
...
- focus-visible: same two-layer gold ring as the primary button
...
- Use `#d4af37` for links, focus, and deliberate featured actions only.
...
- Don't use gold as a general panel background.
...
Build a dark investment-research interface in a black-and-gold palette: a `#000000` canvas, `#0a0a0a` cards, `#1a1a1a` borders, Geist typography, and compact 4px-scale spacing. Use gold `#d4af37` for links, focus rings, and featured actions (hover `#e6c25c`); reserve white for primary text and the primary CTA. Use borders and surface contrast for hierarchy; keep any campaign gradient to a single hero-scale treatment and do not add shadows.